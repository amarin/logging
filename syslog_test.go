...
func TestWrite(t *testing.T) {
	...
	for _, msg := range testMessages {
		...
		defer srvWG.Wait()
		defer sock.Close()
		l, err := NewSyslogWriter("udp", addr)
		if err != nil {
...
func TestConcurrentWrite(t *testing.T) {
	...
	defer srvWG.Wait()
	defer sock.Close()
	s, err := NewSyslogWriter("udp", addr)
	if err != nil {
		t.Fatalf("NewConnSyncer() failed: %v", err)
	}
...
func TestConcurrentReconnect(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		go func() {
			defer wg.Done()
			s, err := NewSyslogWriter("tcp", addr)
			if err != nil {
			...
			for i := 0; i < M; i++ {
				...
				if _, err = io.WriteString(s, testMessage); err != nil {
					t.Errorf("WriteString() failed: %v", err)
					return
				}
			}
...
func TestFailedToConnect(t *testing.T) {
	...
	_, err := NewSyslogWriter("tcp", addr)
	if err == nil {
